...
Chicken.Param.prototype.find_and_replace = function (tmpl, selector, value) {
    var applicator = function (e, val) {
        switch (val.constructor) {
            case Chicken.Param.Attributes:
                e.attr(val.attrs);
                break;
            case Chicken.Param.EventHandlers:
                for (var event in val.events) {
                    e.bind( event, val.events[ event ] );
                }
                break;
            case Function:
                val(e);
    ...
    this.find(tmpl, selector).each(function () {
        var $e = jQuery(this);
        jQuery.map(
            (value instanceof Chicken.Param.Combinator)
                ? value.params
                : (value instanceof Array)
                    ? value
                    : [ value ],
            function (x) { applicator( $e, x ) }
        );
    });
}
...
// ------------------------------------------------------------------
// Chicken.Param takes any type of object as the value, normally
// that value is just a string to replace, but if you pass in an
// instance of Chicken.Param.Attributes, then it will replace the
// attributes instead of replacing the HTML
// ------------------------------------------------------------------
